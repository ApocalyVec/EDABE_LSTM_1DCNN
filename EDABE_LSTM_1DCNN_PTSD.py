""" Automatic EDA artifact correction using a deep learning model.

Running the script:
* Make sure you change the data_root to your data directory.
This script expects the data_root have this structure:
------
<data_root>/
    ├── <subject name 0 †>/
    │   ├── <subject name 0 †>-T1_acq_ledalab.mat
    ├── <subject name 1>/
    │   ├── <subject name 1 †>-T1_acq_ledalab.mat
    ├── ...

† Note that the subject name can be C1-1-Ren-60 or C1-1-Ren_55, notice the difference in the dash and underscore
  connecting the subject name and the session number. This is case for the folder names under data_root.
  However, the .mat file under each folder is always like C1-1-Ren-55-T1_acq_ledalab.mat. Notice it always use dash
  regardless of the folder name.

IMPORTANT NOTE:
* this automatic artifact correction code expects the data to be sampled at 128 Hz.
  Please make sure that the data is sampled at 128 Hz before running this code.
...
def automatic_EDA_correct(df, model,
    ...
    for ctr_it in range(len(end_bad_elements)):
        ...
        spline = sc_int.CubicSpline(x_to_spline, y_to_spline)
        y_output = spline(x_int)

        mix_curve = np.mean([intermediam_correct_lineal, y_output], axis=0)

        tuple_concat = (mix_curve, final_correct) if init_correct.shape[0] < 2 else (
...
for i, d_file in enumerate(data_files):
    ...
    file_name = d_file.split('/')[-1].replace('.mat', '_ArtifactCorrected.mat')
    save_path = os.path.join(os.path.dirname(d_file), file_name)
    # save the mat file
